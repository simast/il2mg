...
type DataFormats = {
	readonly [extension: string]: ((content: string) => any) | undefined;
};
...
export class Data {
	...
	constructor() {
		...
		addLazyProperty(this, 'languages', () =>
			this.load<DataLanguages>('languages'),
		);
		addLazyProperty(this, 'vehicles', () =>
			this.load<DataVehicles>('vehicles'),
		);
		addLazyProperty(this, 'clouds', () => this.load<DataClouds>('clouds'));
		addLazyProperty(this, 'time', () => this.load<DataTime>('time'));
		addLazyProperty(this, 'callsigns', () =>
			this.load<DataCallsigns>('callsigns'),
		);
		...
		addLazyProperty(this, 'countries', () => {
			...
			getEnumValues(Country).forEach(countryId => {
				...
				addLazyProperty(country, 'formations', () =>
					this.load<DataCountryFormations>(
						path.join(countryPath, 'formations'),
					),
				);
		...
		addLazyProperty(this, 'battles', () => {
			...
			for (const battleId in battles) {
				...
				addLazyProperty(battle, 'units', () => {
					...
					const unitsData = this.load<DataBattleUnits>(
						path.join(battlePath, 'units'),
					);
		...
		if (
			...
			if (result === undefined) {
				...
				const directoryDataFiles = fs
					...
					.filter(fileName => {
						const isDirectory = fs
							.lstatSync(path.join(dataItemPath, fileName))
							.isDirectory();
				...
				if (directoryDataFiles.length) {
					result = directoryDataFiles.reduce(
						(result, fileKey) => ({
							...result,
