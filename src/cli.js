...
// --version output
params.version(
	APPLICATION_NAME + ' r' + APPLICATION_VERSION + ' ' + APPLICATION_COPYRIGHT,
);
...
params.option(
	...
	(() => {
		...
		desc +=
			'\tValid date values will depend on the selected battle:' + EOL + EOL;
		...
		desc +=
			EOL + '\tDate can also be specified using special season values:' + EOL;
		desc += EOL + '\t';
...
params.option(
	...
	(() => {
		...
		desc +=
			'\tPilot rank can be specified by prefixing the name with a number and' +
			EOL;
		desc +=
			'\ta comma character. The rank number depends on the country, but always' +
			EOL;
		desc +=
			'\tstarts from 1 and is incremented for each rank in a hierarchy.' +
			EOL +
			EOL;
...
params.option(
	...
	(() => {
		...
		desc +=
			'\t"' +
			FlightState.Start +
			'" - Start from parking area or taxiway (default).' +
			EOL;
		...
		try {
			...
			if (params.date) {
				if (typeof params.date === 'object') {
					...
					if (
						enumContainsValue(MapSeason, params.date) &&
						params.date !== MapSeason.Desert
					) {
						isDateSeason = true;
					}
			...
			if (params.weather !== undefined) {
				const weatherState = Object.keys(WeatherState).find(
					state => state.toLowerCase() === params.weather.toLowerCase(),
				);
			...
			log.D(
				mission.title + ' (' + mission.planes[mission.player.plane].name + ')',
			);
			process.exit();
		} catch (error) {
