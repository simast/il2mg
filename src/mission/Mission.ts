...
export class Mission {
	...
	public readonly make: Array<() => void> = [];
	...
	private createComplexSeed(
		...
		for (
			let paramIndex = 1;
			paramIndex < complexSeedParams.length;
			paramIndex++
		) {
	...
	private saveBinary(fileName: string): Promise<void> {
		...
		const promise = new Promise<void>((resolve, reject) => {
			...
			(function walkItems(items: Immutable<Item[]>): void {
				items.forEach(item => {
