...
export abstract class MCU extends Item {
	...
	public addEvent(type: string, target: MCU): void {
		...
		const eventItem = new MCU.OnEvents.OnEvent(
			this.EVENTS[type]!,
			target.Index!,
		);
	...
	public addReport(type: string, command: MCU, target: MCU): void {
		...
		const reportItem = new MCU.OnReports.OnReport(
			this.REPORTS[type]!,
			command.Index!,
			target.Index!,
		);
	...
	protected writeEvents(buffer: SmartBuffer): void {
		...
		const eventItems =
			(events &&
				events.items &&
				events.items.filter(
					(event): event is MCU.OnEvents.OnEvent =>
						event instanceof MCU.OnEvents.OnEvent,
				)) ||
			[];
		...
		const reportItems =
			(reports &&
				...
				reports.items.filter(
					(report): report is MCU.OnReports.OnReport =>
						report instanceof MCU.OnReports.OnReport,
				)) ||
			[];
...
export namespace MCU.OnEvents {
	...
	export class OnEvent extends Item {
		constructor(public Type: number, public TarId: number) {
			super();
		}
	}
}
